...
d=0;d<a.mfv.length;d++)a.myTableBody.find("#"+a.mfv[d]).addClass(a.favouriteClass);a.profileEnder("buildRows")},highlightFilteredRows:function(){a.profileStarter("highlightFilteredRows");a.myTableBody.find("."+a.selectedFilterItem).each(function(b,c){jQuery(c).removeClass(a.selectedFilterItem)});Object.keys(a.cfi).forEach(function(b,c){for(var d=a.cfi[b],e=[],f=0;f<d.length;f++)e.push(d[f].vtm);a.myTableBody.find("["+a.filterItemAttribute+'="'+b+'"]').each(function(b,c){var d=jQuery(c).text().toString().raw2safe().toLowerCase();
for(b=0;b<e.length;b++)if(d===e[b]){jQuery(c).addClass(a.selectedFilterItem);break}})});a.profileEnder("highlightFilteredRows")},createPagination:function(b,c,d){0<c?jQuery("."+a.noMatchMessageClass).hide():jQuery("."+a.noMatchMessageClass).show();d=b+d;var e=Math.ceil(c/a.visibleRowCount),f=Math.floor(a.sfr/a.visibleRowCount),h="";if(1<e){var k,m=0,p=f-2,l=f+2,v=[];for(k=0;k<e;k++){if(f===k)m=0,h+="<span>["+(k+1)+"]</span>";else{var x=k>=e-1,y=1>k;k>p&&k<l||x||y?(v.length=0,v=[],m=0,0===k&&v.push("first"),
k===e-1&&v.push("last"),k===f-1&&v.push("prev"),k===f+1&&v.push("next"),h+='<a href="#" class="'+v.join(" ")+'"data-page="'+k+'">'+(k+1)+"</a> "):3>m&&(m++,h+=" . ")}h+=" "}a.myTableHolder.find(a.moreRowEntriesSelector).show()}b++;a.myRowsTotalCount===c?(a.myTableHolder.find(a.filteredCountHolder).hide(),a.myTableHolder.find(a.unfilteredCountHolder).show()):(a.myTableHolder.find(a.unfilteredCountHolder).hide(),a.myTableHolder.find(a.filteredCountHolder).show());a.myTableHolder.find(a.minRowSelector).text(b);
