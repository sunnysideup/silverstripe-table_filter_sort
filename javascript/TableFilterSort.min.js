...
function(){var b=a.myTableHolder.find(a.visibleRowCountSelector).val(),b=a.visibleRowCountValidator(b);!1!==b&&a.visibleRowCount!==b&&(a.visibleRowCount=b,window.localStorage.setItem("TFS_visibleRowCount",a.visibleRowCount),a.gotoPage(0,!0))},a.millisecondsBetweenActionsLong)})},setupMoreDetailsListener:function(){a.myTableHolder.on("click",a.showMoreDetailsSelector,function(b){b.preventDefault();b=jQuery(this);b.toggleClass(a.openedClass);b.closest("tr").toggleClass(a.openedClass).find(a.moreDetailsSelector).each(function(b,
...
for(b=b=0;b<c.length;b++){var f=c[b],u=a.myFilterFormInner.find('input[name="'+f+'"].awesomplete').first(),d=jQuery(u).attr("id"),d=document.getElementById(d);u.on("input",function(a,b){console.log(u.next("ul"))});new Awesomplete(d,{list:a.dataDictionary[f].Options,autoFirst:!0,filter:function(a,b){return Awesomplete.FILTER_CONTAINS(a,b)&&-1===Awesomplete.blackList.indexOf(a.value)},replace:function(b){a.makeCheckboxSection(this.input,b.value);this.input.value="";Awesomplete.blackList.push(b.value);
a.runCurrentFilter()}});d.addEventListener("awesomplete-close",function(a){if("nomatches"===a.reason){var b=jQuery(a.srcElement).val();jQuery(a.srcElement).val(b.slice(0,-1))}});Awesomplete.blackList=[]}}a.profileEnder("createFilterForm")},makeSectionHeaderForForm:function(b,c){var d=b+a.sectionFilterClassAppendix;c.replace(/\W/g,"");d='<div class="'+a.filterGroupClass+" "+d+'" field-type="'+b+'" data-to-filter="'+c.raw2attr()+'">';"checkbox"===b&&(d+='<span class="'+a.inverseSelectionFilterClass+
...
!1,k=!0,n=0;n<a.cfi[b].length;n++){var h=a.cfi[b][n];if(!0===k){if("Favourites"===b)"undefined"!==typeof c&&0<c.length&&-1<a.mfv.indexOf(c)&&(g=!0);else if(Array.isArray(a.dataDictionary[b].Values[c]))for(var l=a.dataDictionary[b].Values[c],m=0;m<l.length;m++){var r=l[m],r=a.validateValue(b,r);if("Keyword"===b){for(var t=h.vtm,r=r.toLowerCase(),t=t.split(" "),v=!0,w=0;w<t.length;w++){var z=t[w].raw2safe().toLowerCase();if(-1===r.indexOf(z)){v=!1;break}}!0===v&&(g=!0)}else switch(a.dataDictionary[b].DataType){case "date":break;
case "number":"undefined"!==typeof h.vtm?(t=h.vtm,r==t&&(g=!0)):(v=h.lt,t=!0,jQuery.isNumeric(v)&&0!==v&&v<r&&(t=!1),t&&(v=h.gt,jQuery.isNumeric(v)&&0!==v&&v>r&&(t=!1)),t&&(g=!0));break;default:r=r.raw2safe().toLowerCase(),r===h.vtm&&(g=!0)}if(g)return!1}g&&(k=!1)}}g||(f=e=!1)}});e?(a.myRowsMatching.push(c),a.useJSON||d.addClass(a.matchClass).removeClass(a.notMatchClass)):a.useJSON||d.addClass(a.notMatchClass).removeClass(a.matchClass)}}a.windowTimeoutStoreSetter("endRowManipulation",function(){a.endRowManipulation()},
...
e[b]){jQuery(c).addClass(a.selectedFilterItem);break}})});a.profileEnder("highlightFilteredRows")},createPagination:function(b,c,d){0<c?jQuery("."+a.noMatchMessageClass).hide():jQuery("."+a.noMatchMessageClass).show();d=b+d;var e=Math.ceil(c/a.visibleRowCount),f=Math.floor(a.sfr/a.visibleRowCount),h="";if(1<e){var k,m=0,p=f-2,l=f+2,q=[];for(k=0;k<e;k++){if(f===k)m=0,h+="<span>["+(k+1)+"]</span>";else{var x=k>=e-1,y=1>k;k>p&&k<l||x||y?(q.length=0,q=[],m=0,0===k&&q.push("first"),k===e-1&&q.push("last"),
k===f-1&&q.push("prev"),k===f+1&&q.push("next"),h+='<a href="#" class="'+q.join(" ")+'"data-page="'+k+'">'+(k+1)+"</a> "):3>m&&(m++,h+=" . ")}h+=" "}a.myTableHolder.find(a.moreRowEntriesSelector).show()}b++;a.myRowsTotalCount===c?(a.myTableHolder.find(a.filteredCountHolder).hide(),a.myTableHolder.find(a.unfilteredCountHolder).show()):(a.myTableHolder.find(a.unfilteredCountHolder).hide(),a.myTableHolder.find(a.filteredCountHolder).show());a.myTableHolder.find(a.minRowSelector).text(b);a.myTableHolder.find(a.maxRowSelector).text(d);
...
m=f.attr("field-type").toLowerCase(),l=[];f.find("input").each(function(b,c){c=jQuery(c);var d=c.val(),f=a.validateValue(h,d);e=d.raw2safe();g=e.toLowerCase();var k,n;switch(m){case "keyword":if(1<g.length){g.replace(" OR ",",");for(var d=g.split(","),p=d.length,f=0;f<p;f++)if(k=d[f].trim(),1<k.length){var u=k;if(Array.isArray(a.keywordToFilterFieldArray)&&0<a.keywordToFilterFieldArray.length)for(n=0;n<a.keywordToFilterFieldArray.length;n++)if(2<k.length){var q=a.keywordToFilterFieldArray[n];-1<a.dataDictionary[q].Options.indexOf(k)&&
("undefined"===typeof a.cfi[q]&&(a.cfi[q]=[]),a.cfi[q].push({vtm:k,ivl:k}),k="")}1<k.length&&(n=k,"undefined"===typeof a.cfi[h]&&(a.cfi[h]=[]),a.cfi[h].push({vtm:n,ivl:k}));l.push(u)}}break;case "tag":case "checkbox":c.is(":checked")&&("undefined"===typeof a.cfi[h]&&(a.cfi[h]=[]),l.push(g),a.cfi[h].push({vtm:g,ivl:e}));break;case "favourites":c.is(":checked")?("undefined"===typeof a.cfi[h]&&(a.cfi[h]=[]),a.cfi[h].push({vtm:g,ivl:e}),l.push(g),a.myTableHolder.addClass(a.hasFavouritesInFilterClass)):
